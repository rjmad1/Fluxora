...
export class IngestionService {
  ...
  async ingestLinkedIn(profileUrl: string) {
    ...
    const extractedData = {
      ...
      summary: '12+ years building distributed cloud infrastructure and AI intelligence layers.',
      roles: [
        { title: 'Principal AI Architect', company: 'Quantum Labs', industry: 'Artificial Intelligence' },
        { title: 'Lead Engineer', company: 'CloudCorp', industry: 'SaaS' }
      ],
      skills: ['TypeScript', 'LangGraph', 'Temporal', 'Vector Search', 'System Design'],
    ...
    await this.profileService.updateProfile({
      ...
      aiMetadata: { source: 'LinkedIn Ingestion', profileUrl, ingestedAt: new Date().toISOString() },
    });

    // 2. Enrich Knowledge Graph Nodes & Edges
    const companyNodes = [];
    for (const role of extractedData.roles) {
      const companyNode = await this.graphService.addNode('Company', role.company, { industry: role.industry });
      companyNodes.push(companyNode);
      await this.graphService.addNode('Expertise', role.title, { level: 'CORE', category: 'EXPERIENCE' });
    }

    for (const skill of extractedData.skills) {
      const skillNode = await this.graphService.addNode('Skill', skill, { level: 'Expert' });
      for (const compNode of companyNodes) {
        await this.graphService.addEdge(skillNode.id, compNode.id, 'UTILIZED_AT');
      }
    }

    for (const interest of extractedData.interests) {
      await this.graphService.addNode('Interest', interest, { category: 'PASSION', level: 'HIGH' });
    }
  ...
  async ingestResume(resumeText: string) {
    this.logger.log(`Ingesting Resume/CV content of length: ${resumeText.length}`);

    // Simple keyword extraction heuristics
    const skillsList = ['NestJS', 'React', 'Kubernetes', 'Apache Kafka', 'PostgreSQL', 'Docker', 'ClickHouse'];
    const foundSkills = skillsList.filter((s) => resumeText.toLowerCase().includes(s.toLowerCase()));
    ...
    for (const skill of foundSkills) {
      await this.graphService.addNode('Skill', skill, { level: 'Advanced', source: 'CV Upload' });
    }
  ...
  async ingestWebsite(url: string) {
    ...
    const extractedTopics = ['Observability', 'Multi-tenant Isolation', 'ClickHouse Aggregations'];
    
    for (const topic of extractedTopics) {
      await this.graphService.addNode('Topic', topic, { source: 'Website crawl', url });
    }
